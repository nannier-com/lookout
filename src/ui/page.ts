/**
 * The page the server hands back, assembled from its parts.
 *
 * It is one string, which is a decision worth stating: the ui is a local dev
 * page for one reader, and a build step that turns modules into a bundle would
 * be more machinery than the thing it serves. What that costs is real, and the
 * test beside it exists because of it: nothing in the build type-checks or
 * lints a template literal, so a stray escape in here reaches the browser and
 * breaks the page with every gate still green.
 */
import { LEARNING_CSS, LEARNING_HTML, LEARNING_JS } from "./page-learning.js";
