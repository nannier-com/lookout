...
/**
 * What the run about to start will do to the application, if anything.
 *
 * The calls-to-action consent lives under the cog now, which means the one
 * place it can still be read at the moment of pressing is this button. It is
 * worth saying, because it is the only thing play can start that touches the
 * application instead of reading it: everything planned gets actuated,
 * destructive controls included.
 */
function willClick(): string {
  return page.config.navigation
    ? " Calls to action are ON: it will click this project's own buttons and links, destructive ones included."
    : "";
}
