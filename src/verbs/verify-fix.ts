...
${issueId}: not ruled. ${what}`);
  ...
  await recordAttempt(
    ...
    attemptRecord({
      ...
      changedShots: changedShots.size,
      baselineShots,
      stillOpen: stillOpen.map((f) => ({ title: f.title, shotId: f.evidence[0]?.shotId ?? "", observed: f.observed })),
      unclosable: [...new Set(unclosable.map((m) => m.route))],
      ...
      observed: await observeRepo(resolved.projectDir, previous?.reported?.commit, reportedCommit),
    }),
  );
  await saveBacklog(resolved, backlog);
