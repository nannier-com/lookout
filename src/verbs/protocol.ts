...
export const WHAT_LOOKOUT_IS = [
  ...
  "who fixes them. It hands one over at a time, to the tool you picked, in the",
  "order you queued them, and then waits to be asked to rule. What to do about a",
  "finding is still your call, not lookout's.",
  "",
...
export function protocolText(): string {
  return [
    ...
    "lookout map                    # read the source for the screens; check walks that map",
    ...
    "incidents lookout has hit in this project and in its own checkout, fixes",
    "the cause of the heaviest one in its own source, and reverts the lot",
    "unless the type check, the linter, the tests and the build all pass",
    "afterwards, plus a replay of a frozen regression set from a project that",
    ...
    "",
    "`lookout map` reads the source for the application's screens and how each",
    "is reached, and writes `.lookout/map.json`: every route and every state a",
    "route can show, what to open on its parent, its risk, and the file that",
    "declares it. Screens it finds beyond the config are walked and in scope;",
    "the config is never edited. With a map, `check` walks the application",
    "one screen at a time: reach it, capture it across the matrix, judge it, file",
    "its findings, then the next. A route is opened by URL; a state is reached",
    "by replaying what reached it before, or by a navigator (an AI handed",
    "lookout's own tool server over the page or a booted simulator) when nothing",
    "has, and what it did is recorded for the next run. An unreachable screen is",
    "reported and never counted clean. `--first` stops at the first screen with",
    "standing findings; `--no-map` captures the matrix instead; a stale map is",
    "walked with a warning until `lookout map` runs again (`--map` lets `check`",
    "scan for one itself). `--replay-only`, `--max-screens`, `--budget-usd` and",
    "`--max-navigator-calls` bound a walk.",
    "",
